...
final class DirectoryWatcherTests: XCTestCase {
    ...
    func testFilteredDirectorySnapshotsCanBeDisabled() throws {
        var config = DirectoryWatcher.Configuration()
        config.scansChangedDirectoriesForFilteredEvents = false
        config.filterChain.add(.fileExtensions(["txt"]))

        watcher = try DirectoryWatcher(url: testDirectory, configuration: config)

        let directoryExpectation = XCTestExpectation(description: "Directory change detected")
        let filteredExpectation = XCTestExpectation(description: "Filtered snapshot skipped")
        filteredExpectation.isInverted = true

        watcher.onDirectoryChange = { url in
            XCTAssertEqual(url, self.testDirectory)
            directoryExpectation.fulfill()
        }

        watcher.onFilteredChange = { _ in
        ...
        let testFile = testDirectory.appendingPathComponent("test.txt")
        try "Text content".write(to: testFile, atomically: true, encoding: .utf8)

        wait(for: [directoryExpectation, filteredExpectation], timeout: 2.0)
    }
